...
exports.run = async (client, message, args) => {
  ...
  if (userToForceLeave.permissions.has("ADMINISTRATOR" || "MANAGE_ROLES")) {
    return message.channel.send(
      "`" + message.author.username + "`" + " you cannot blacklist this user!"
    );
  }
  ...
  if (isUserBlacklist) {
    return message.channel.send(
      "`" + message.author.username + "`" + " that user is already blacklisted"
    );
  }
  if (
    userToForceLeave.roles.cache.some((role) => role.id === messageRole.role)
  ) {
    userToForceLeave.roles.remove(messageRole.role).catch((err) => console.err);
  }
