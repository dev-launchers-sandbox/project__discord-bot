...
function formatLeaderboard(
	...
	if (devBeansArray.length !== 0) {
		devBeansArray.forEach((useEntry) => {
			...
			if (!userObject)
				userObject = { user: { username: "User not found" } };
	...
	if (goldenBeansArray.length !== 0) {
		...
		if (goldenBeansArray.length > maxSize)
			maxSize = goldenBeansArray.length;
	} else {
