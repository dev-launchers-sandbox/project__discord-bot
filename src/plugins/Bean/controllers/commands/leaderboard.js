...
const getSortedDevBeansArray = (message, accounts) => {
  ...
  Object.keys(accounts).forEach((userID) => {
    ...
    if (account.devBeansTwo) {
      devBeansArray.push({ id: userID, type: account.devBeansTwo });
    }
  });
  ...
  Object.keys(accounts).forEach((userID) => {
    let account = accounts[userID];
    if (account.goldenBeansTwo) {
      goldenBeansArray.push({ id: userID, type: account.goldenBeansTwo });
    }
  });
