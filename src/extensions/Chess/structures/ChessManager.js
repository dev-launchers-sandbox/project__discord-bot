module.exports = class {
		...
		for (let i = 0; i < coords.length; i++) {
			if (!board[i]) {
				...
				let data = fs.readFileSync(
					`./src/extensions/Chess/assets/pieces/${
						board[i].color
					}${board[i].type.toUpperCase()}.png`
				);
				let img = new Image();
				img.src = data;
				...
				this.tempChannel.sendEmbed({
					title: `**${
						client.users.cache.get(
							this.getGame(message.author.id).players[0]
						).username
					} vs. ${
						client.users.cache.get(
							this.getGame(message.author.id).players[1]
						).username
					}**`,
					color: 0xff9f01,
	...
	abort(message) {
		if (this.removeGame(message, message.author.id) === false) {
			...
			return message.reply(
				"I have aborted the game and deleted the channel."
			);
		}
	}
			...
			if (message.author.id != game.turns[chessGame.turn()]) {
				...
				if (chessGame.moves().indexOf(move) === -1) {
					...
					if (chessGame.game_over()) {
						this.event = `**${
							chessGame.turn() === "w"
								? client.users.cache.get(game.players[1])
										.username + " (Black)"
								: client.users.cache.get(game.players[0])
										.username + " (White)"
